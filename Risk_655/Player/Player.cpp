...
Player::Player(std::string playersName, int strategyId) : name(playersName), ownedCountries(new list<Country*>), ownedContinents(new list<Continent*>), numOfOwnedCountries(0), numOfOwnedContinents(0), numCardsArtillery(0), numCardsCavalry(0), numCardsInfantry(0), playersArtilleryCards(new list<Card*>), playersCavalryCards(new list<Card*>), playersInfantryCards(new list<Card*>), playersWildCards(new list<Card*>) {
	computer = true;
	numberBattlesWon = 0;
...
void Player::setStrategy(int strategyId) {
	if (strategy != NULL)
		delete strategy;
	computer = true;
	switch (strategyId) {
...
void Player::placeArmiesOnCountriesLoop(int totalNumArmies) {
	...
	while (totalNumArmies != 0) {
		cout << name << ", you have " << totalNumArmies << " armies to place on any of the following countries: " << endl;
		cout << "COUNTRY ID\tCOUNTRY NAME\tCURRENT # OF ARMIES" << endl;
		for (list<Country*>::iterator i = ownedCountries->begin(); i != ownedCountries->end(); ++i) {
			cout << (*i)->getId() << "\t\t" << (*i)->getName() << "\t\t" << (*i)->getArmies() << endl;
		}
...
void Player::setArmiesInCountry(Country* c, int n) {
	c->setArmies(n);
	if (isComputer()) {
		if (getTotalNumArmies() < (getNumOfOwnedCountries() + 2)) {
			if (strategy->getType() == "Agressive") {
				setStrategy(2);
				notify(STRATEGY_ID);
			}
		}
		else if (getTotalNumArmies() > (getNumOfOwnedCountries() * 5)) {
			if (strategy->getType() == "Defensive") {
				setStrategy(1);
				notify(STRATEGY_ID);
			}
		}
