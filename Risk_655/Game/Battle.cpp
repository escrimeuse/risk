...
void Battle::doBattle(vector<Player*>& players, Map* map) {
	...
	while (continueFight) {
		...
		if ((defender.country)->getArmies() == 0) {
			...
			attacker.player->setNumberBattlesWon(attacker.player->getNumberBattlesWon() + 1);
			...
			if ((attacker.country)->getArmies() == 2) {
				...
				if (defender.player->isAlive()) {
					...
					// transfer all of the defender player's cards to the attacker player
					
					for (list<Card*>::iterator i = defender.player->getPlayersArtilleryCards()->begin(); i != defender.player->getPlayersArtilleryCards()->end(); ++i) {
						attacker.player->addPlayerCard(*i);
					}
					for (list<Card*>::iterator i = defender.player->getPlayersCavalryCards()->begin(); i != defender.player->getPlayersCavalryCards()->end(); ++i) {
						attacker.player->addPlayerCard(*i);
					}
					for (list<Card*>::iterator i = defender.player->getPlayersInfantryCards()->begin(); i != defender.player->getPlayersInfantryCards()->end(); ++i) {
						attacker.player->addPlayerCard(*i);
					}
					for (list<Card*>::iterator i = defender.player->getPlayersWildCards()->begin(); i != defender.player->getPlayersWildCards()->end(); ++i) {
						attacker.player->addPlayerCard(*i);
					}
	...
	while (attack) {
		...
		if (canAttack) {
			...
			if(!player->isComputer()){
				...
				cout << player->getName() << " is a Computer" << endl;
