...
int Battle::selectAttackingCountry() {
	...
	Country** countries = (attacker.player)->getOwnedCountries();		
...
void Battle::decideNumDiceToRoll(bool allIn) {
	...
	if ((attacker.country)->getNumOfArmies() == 2) {
		cout << endl << (attacker.player)->getName() << " will roll 1 dice" << endl;
		attacker.numOfDice = 1;
	}
	else if ((attacker.country)->getNumOfArmies() == 3) {
	...
	if ((defender.country)->getNumOfArmies() == 1 || (defender.country)->getNumOfArmies() == 2) {
		cout << endl << (defender.player)->getName() << ", you will roll 1 die" << endl;
		defender.numOfDice = 1;
	}
	else if ((defender.country)->getNumOfArmies() > 2) {
...
void Battle::compareRolls() const {
	...
	for (int i = 0; i < numDiceToCompare; i++) {
		...
		if (attacker.dice[i] > defender.dice[i]) {
			...
			int numOfArmiesCurrentlyOnDefendingCountry = (defender.country)->getNumOfArmies() -1;
			(defender.country)->setNumOfArmies(numOfArmiesCurrentlyOnDefendingCountry);
...
void Battle::doBattle() {
	...
	while (continueFight) {
		...
		if ((defender.country)->getNumOfArmies() == 0) {
			...
			if ((attacker.country)->getNumOfArmies() == 2) {
				...
				(defender.country)->setNumOfArmies(1);
				(attacker.country)->setNumOfArmies(1);
			}
			else {
				int maxNumArmiesToPlace = (attacker.country)->getNumOfArmies() - 1;
				...
				if (allIn) {
					...
					(defender.country)->setNumOfArmies(maxNumArmiesToPlace);
					...
					(defender.country)->setNumOfArmies(numArmiesToPlace);

					// change the number of armies on the attacking country as appropriate
					(attacker.country)->setNumOfArmies((attacker.country)->getNumOfArmies() - numArmiesToPlace);
		...
		else if ((attacker.country)->getNumOfArmies() == 1) {
		...
		else if ((attacker.country)->getNumOfArmies() >1 && !allIn) {
...
void attackPhase(Player* player) {
	...
	while (attack) {
		...
		for (int i = 0; i < player->getNumOfOwnedCountries(); i++) {
			if (playersCountries[i]->getNumOfArmies() >= 2) {
				canAttack = true;
			}
		}
