...
namespace mapDemo {
	...
	static void welcome() {
		cout << "==========================================\n"
			<< "Welcome to Jennifer's Map-testing program!\n"
			<< "==========================================\n"
	...
	static Country* makeCountry() {
		...
		while (true) {
			...
			if (myStream >> userNum) {
				...
				cout << "There are " << newCountry->getArmies() << " armies in " << newCountry->getName() << ".\n\n"
					<< "Congratulations, " << newCountry->getName() <<" is officially a country!" << endl;
				return newCountry;
				break;
			}
			else
	...
	static Map* makeMap() {
		...
		cin >> mapName;
		Map *newMap = new Map(mapName,9,3);
		cout << "\nCurrently, there is only one size of map available. " << newMap->getName() << " will have 3 continents, with 3 countries in each.\n\n"
		...
		cout << "All continents have been named. The names are: " << newMap->getContinentNames() << "\n"
			<< "Please name your 9 countries: " << endl;
